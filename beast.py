while(True):
	...
	def build_level():
		...
		global board_rows, board_cols, reset_board, blank_board, board, beast_cnt, monster_cnt, egg_cnt, key_move, level, lives, score, points, BAKGRD, BLOCK, KILLBLOCK, last_frame
	...
	def take_input():
		...
		while(True):
			...
			timeout = 0
	#		system('echo \'' + str(keypress) + '\' >> keypresscodes.txt')
			if keypress == ord('q'):
				system('clear')
				exit()
			elif keypress == 27:
